...
def test_prod_alt_domains(
    ...
    """Prueba de prod todos los dominios se producen correctamente."""
    copy(
        ...
        data={
            ...
            "domain_prod": "main.ejemplo.com",
            "domain_prod_alternatives": ["alt0.ejemplo.com", "alt1.ejemplo.com"],
        },
    )
...
@pytest.mark.parametrize("backup_dst", (None, "s3://ejemplo", "sftp://ejemplo"))
@pytest.mark.parametrize("smtp_relay_host", (None, "ejemplo"))
def test_backup_config(
    ...
    if not backup_dst:
        ...
        return
    # Check selected safety image
    if backup_dst == "s3://ejemplo":
        assert prod["services"]["backup"]["image"] == "dued/safety:postgres-s3"
    else:
        assert prod["services"]["backup"]["image"] == "dued/safety:postgres"
